 ...
 * @returns {string[]} JavaScript files below the directory.
 */
...
for (const root of ["src", ".testbed"]) {
  for (const file of collectJavaScriptFiles(root)) {
    ...
    if (/(?:from|import)\s*\(?\s*["'][^"']*\/(?:\.testbed|testbed)\//u.test(source)) {
      failures.push(`${file}: imports a sibling testbed surface`);
    }
  }
}
...
console.log("Public import-boundary foundation check passed.");
