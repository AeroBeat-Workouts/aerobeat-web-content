...
- Playable content uses `aerobeat.song-package.v5` / package `5.0.0`, with exact hash-bound `source.spawnTiming` raw BPM/NJS/offset and recomputed `beatsaber_core_hjd_v1` HJD/reaction-time/jump-distance values. Package versions 1–4 fail with stable `spawn_timing_reimport_required`. A non-enumerable generation-bound private content seam exposes the immutable validated spawn timing only while ready; it never enters public snapshots, telemetry, or serialization. The package retains exactly one palette-bound normalized-obstacle `aerobeat.chart.flow.v4` chart and the frozen four-chart Boxing v1 recipe/ruleset matrix. A non-enumerable generation-bound private assembly seam exposes the immutable validated effective palette while ready; stale, loading, idle, and destroyed generations receive `null`, and no palette data enters public snapshots or serialization.
